...
export interface WebElementProps extends ElementOrFragmentProps<WebElement> {
  ...
  search(text?: string): Promise<WebElement>;
  typeIn(text?: string): Promise<WebElement>;
...
export class WebElement implements WebElementProps {
  ...
  public async search(text?: string): Promise<WebElement> {
    if (text) {
  ...
  public async typeIn(text?: string): Promise<WebElement> {
    if (text) {
      await playwrightPageLocator.type(text);
    } else if (!text && this.webElementProps.text) {
      await playwrightPageLocator.type(this.webElementProps.text);
