...
export class TodoMvcPage extends WebFragment {
  ...
  async verifyFilterTodosClassWithText(
    filterText: string,
  ...
  async verifyClearCompletedIsHidden() {
    await this.webElement(
      TodoMvcPageLocators.clearCompletedLabelLocator
    ).verifyActionable(Actionable.ToBeHidden);
  }
  ...
  async verifyATodoItemClass(nth: number, todoClass: string | string[]) {
    await this.waitForNthWebElement(
      TodoMvcPageLocators.todoItemsLocator,
      nth,
      Actionable.ToBeEnabled
    )
  ...
  async verifyATodoToggleNotVisible(nth: number) {
    let toggleATodoBtn: WebElement = TodoMvcPageVars.nthTodoItem(nth);
    await toggleATodoBtn
      .findInLocator('.toggle')
      .verifyNotActionable(Actionable.ToBeVisible);
  }
  ...
  async verifyIfMainElementVisible() {
    await this.webElement(
      TodoMvcPageLocators.mainElementLocator
    ).verifyActionable(Actionable.ToBeVisible);
  }

  async verifyIfToggleAllIsChecked() {
    await this.webElement(
      TodoMvcPageLocators.toggleAllBtnLocator
    ).verifyActionable(Actionable.ToBeChecked);
  }

  async verifyIfToggleAllIsNotChecked() {
    await this.webElement(
      TodoMvcPageLocators.toggleAllBtnLocator
    ).verifyNotActionable(Actionable.ToBeChecked);
  }

  async verifyIfFooterElementVisible() {
    await this.webElement(
      TodoMvcPageLocators.footerElementLocator
    ).verifyActionable(Actionable.ToBeVisible);
  }
